...
function DrawSpiral(context) {
    ...
    for (let counter = 3; counter < 600; counter += 3) {
        ...
        context.stroke();
        turnRight(100); // Changing degree from 89 to 100 for increasing rotating to the right in order to get our pattern
    }
}

function drawRisunok() {
            const context = canvas.getContext('2d');
            context.clearRect(0, 0, canvas.width, canvas.height);
            const colors = ['#123456', '#821221', '#ffd5ff', '#ffffd3'];
            for (let i = 0; i < 100; i++) {
